...
export default function SplitPane({ left, right }) {
  const [leftWidth, setLeftWidth] = useState(50); // desktop %
  const [isDesktop, setIsDesktop] = useState(window.innerWidth >= 768);
  const dragging = useRef(false);

  // Watch for window resize to update layout mode
  useEffect(() => {
    const handleResize = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className="
          h-1/2 md:h-full
          w-full md:overflow-auto bg-white
        "
        style={{
      ...
      {isDesktop && (
        <div
          className="
            w-1 bg-gray-300 hover:bg-gray-400 cursor-col-resize transition-colors
          "
